...
public class ThumbsDown
{
  ...
  public String Add_ThumbsDown(@QueryParam("user_id") String user_id, @QueryParam("post_id") String post_id, @QueryParam("InfringingUserId") String InfringingUserId, @QueryParam("IsTwitterPost") boolean IsTwitterPost, @QueryParam("IsComment") boolean IsComment)
    throws Exception
  {
    ...
    if (IsTwitterPost) { rs = st.executeQuery("select TweetID as post_id, User_ID as user_id from TwitterThumbsDown where  TweetID = '" + post_id + "' and User_ID ='" + user_id + "';");
    }
    else if (IsComment) { rs = st.executeQuery("select post_id,  commentId, PcommentId, user_id  from commentsThumbdown where post_id = '" + idArray[0] + "' and PcommentId = '" + idArray[1] + "' and commentId = '" + idArray[2] + "' and user_id = '" + user_id + "';");
    } else {
      rs = st.executeQuery("select post_id, user_id from thumbsDown where  post_id = '" + post_id + "' and User_ID ='" + user_id + "';");
    }
    if ((RecordPresent = checkIfRecordAlreadyPresent(rs, post_id, user_id)))
    {
      if (IsTwitterPost)
      {
  ...
  public boolean checkIfRecordAlreadyPresent(ResultSet rs, String post_id, String user_id)
    throws SQLException
  {
    boolean RecordPresent = false;
