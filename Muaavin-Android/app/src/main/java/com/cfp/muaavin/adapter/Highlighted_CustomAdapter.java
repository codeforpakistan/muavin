...
public class Highlighted_CustomAdapter extends BaseAdapter {
    ...
    HighlightedFragment BrowseActivityDelegate;


    private static LayoutInflater inflater=null;

    public Highlighted_CustomAdapter(Context context, HighlightedFragment browser_activity, HashMap<String, ArrayList<PostDetail>> post_details) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        View rowView;
        rowView = inflater.inflate(R.layout.highlight_row_layout, null);
...
*/
        ...
        if((float)result.get(keys.get(position)).get(0).unlike_value > (float)(result.get(keys.get(position)).get(0).count/10))
            holder.PostHeading.setTextColor(context.getResources().getColor(R.color.red));
