...
*/
            if (check == 0)
                new WebHttpGetReq(context, null, check, null, this).execute(MenuActivity.baseURL + "Muaavin-Web/rest/posts/Insert_Post?user_name=" + AesEncryption.encrypt(Profile.getCurrentProfile().getName()) + "&UserState=" + AesEncryption.encrypt(User.getLoggedInUserInformation().state) + "&ReportedUserState=" + AesEncryption.encrypt(FacebookUtil.ReportPostDetail.user_state) + "&Post_id=" + AesEncryption.encrypt(FacebookUtil.ReportPostDetail.post_id) + "&Group_id=" + AesEncryption.encrypt(String.valueOf(FacebookUtil.ReportPostDetail.group_id)) + "&Comment_id=" + AesEncryption.encrypt(FacebookUtil.ReportPostDetail.coment_id) + "&PComment_id=" + AesEncryption.encrypt(FacebookUtil.ReportPostDetail.ParentComment_ID) + "&Group_name=" + AesEncryption.encrypt(Group_name) + "&Profile_name=" + AesEncryption.encrypt(Profile.getCurrentProfile().getId()) + "&user_id=" + AesEncryption.encrypt(FacebookUtil.ReportPostDetail.infringing_user_id) + "&infringing_user_name=" + AesEncryption.encrypt(FacebookUtil.ReportPostDetail.infringing_user_name) + "&Post_image=" + AesEncryption.encrypt(FacebookUtil.ReportPostDetail.post_image) + "&userProfilePic=" + AesEncryption.encrypt(user.profile_pic) + "&infringingUser_ProfilePic=" + AesEncryption.encrypt(FacebookUtil.ReportPostDetail.infringing_user_profile_pic) + "&Comment=" + AesEncryption.encrypt(FacebookUtil.ReportPostDetail.comment) + "&Post_Det=" + AesEncryption.encrypt(FacebookUtil.ReportPostDetail.post_Detail));
            else if (check == 1)
                new WebHttpGetReq(context, null, check, null, this).execute(MenuActivity.baseURL + "Muaavin-Web/rest/Users/Highlights?name=" + AesEncryption.encrypt(Group_name) + "&user_id=" + AesEncryption.encrypt(Profile.getCurrentProfile().getId()) + "&specificUserFriends=" + true);
            else if (check == 2) {
                Browse_Activity frag = new Browse_Activity();
                Bundle args = new Bundle();
                args.putString("Group_name", Group_name);
                args.putBoolean("isFb", true);
                ...
                Bundle args = new Bundle();
                args.putString("Group_name", Group_name);
                args.putBoolean("isFb", false);
                frag.setArguments(args);
                FragmentManager fragmentManager = activity.getFragmentManager();
                fragmentManager.beginTransaction().replace(R.id.fragment_container, frag).addToBackStack(null).commit();
            } else if (check == 1000) {
                HighlightedFragment frag = new HighlightedFragment();
                Bundle args = new Bundle();
                args.putString("Group_name", Group_name);
                args.putBoolean("isFb", true);
                frag.setArguments(args);
                ...
                BrowsePost_ListView frag = new BrowsePost_ListView();
                Bundle args = new Bundle();
                args.putString("Group_name", Group_name);
                args.putString("user_id", Profile.getCurrentProfile().getId());
                frag.setArguments(args);
                FragmentManager fragmentManager = activity.getFragmentManager();
                fragmentManager.beginTransaction().replace(R.id.fragment_container, frag).addToBackStack(null).commit();
            }
        ...
        */
            if (check == 5)
                new WebHttpGetReq(context, activity, check, null, this).execute(MenuActivity.baseURL + "Muaavin-Web/rest/TweetQuery/AddTweet?User_ID=" + AesEncryption.encrypt(User.getTwitterUserLoggedInInformation().id) + "&User_Name=" + AesEncryption.encrypt(User.getTwitterUserLoggedInInformation().name) + "&User_ImageUrl=" + AesEncryption.encrypt(UrlHelper.getEncodedUrl(User.getTwitterUserLoggedInInformation().profile_pic)) + "&Tweet_ID=" + AesEncryption.encrypt(TwitterUtil.ReportTwitterDetail.post_id) + "&Message=" + AesEncryption.encrypt(TwitterUtil.ReportTwitterDetail.post_Detail) + "&ImageUrl=" + AesEncryption.encrypt(TwitterUtil.ReportTwitterDetail.post_image) + "&Group_Name=" + AesEncryption.encrypt(Group_name) + "&InfringingUserID=" + AesEncryption.encrypt(TwitterUtil.ReportTwitterDetail.infringing_user_id) + "&InfringingUserName=" + AesEncryption.encrypt(TwitterUtil.ReportTwitterDetail.infringing_user_name) + "&InfringingUserProfilePic=" + AesEncryption.encrypt(TwitterUtil.ReportTwitterDetail.infringing_user_profile_pic));
            else if (check == 7)
                new WebHttpGetReq(context, activity, check, null, this).execute(MenuActivity.baseURL + "Muaavin-Web/rest/Users/Highlights?name=" + AesEncryption.encrypt(Group_name) + "&user_id=" + AesEncryption.encrypt(User.getTwitterUserLoggedInInformation().id) + "&specificUserFriends=" + true + "&isTwitterData=" + true);
        }
    }
    ...
    public void getUserAndPostData(ArrayList<Post> results, String option) {
        ...
        if (option.equals("GroupPosts")) {
            if (FacebookUtil.Posts.size() > 0) {
                ...
                Bundle args = new Bundle();
                args.putBoolean("isTwitterData", false);
                args.putBoolean("ClipBoardOption", false);
                args.putBoolean("GroupPostOption", true);
                args.putString("User_id", Profile.getCurrentProfile().getId());
                args.putSerializable("user_posts", FacebookUtil.Posts);
                frag.setArguments(args);
                FragmentManager fragmentManager = activity.getFragmentManager();
                fragmentManager.beginTransaction().replace(R.id.fragment_container, frag).addToBackStack(null).commit();
            } else {
                DialogBox.showErrorDialog(context, "Error", "Permission Error");
            }
            ...
            if (PostsLoadAsyncTask.getPostResponse() == true) {
                ...
                Bundle args = new Bundle();
                args.putString("User_id", Profile.getCurrentProfile().getId());
                args.putSerializable("user_posts", ClipBoard_Posts);
                args.putBoolean("GroupPostOption", false);
                args.putBoolean("ClipBoardOption", true);
                args.putBoolean("isTwitterData", false);
                frag.setArguments(args);
                FragmentManager fragmentManager = activity.getFragmentManager();
                fragmentManager.beginTransaction().replace(R.id.fragment_container, frag).addToBackStack(null).commit();
...
*/
    ...
    public void getGroups(ArrayList<Group> Groups) {
        GroupsListView frag = new GroupsListView();
        Bundle args = new Bundle();
    ...
    public void getReportedFriends(ArrayList<String> InfringingUserIds, String dataType) {
        ...
        if (dataType.equals("Twitter")) {
            friends = TwitterUtil.Followers;
        }

        for (int j = 0; j < friends.size(); j++) {
            String friend_id = friends.get(j).id;
            if (InfringingUserIds.contains(friend_id)) {
